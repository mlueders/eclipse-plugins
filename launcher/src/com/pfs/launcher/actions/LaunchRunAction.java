...
public class LaunchRunAction extends BindableLaunchAction implements IWorkbenchWindowPulldownDelegate {
	...
	private BoundRunHistoryMenuAction runHistoryMenuAction;
	...
	public void dispose() {
		...
		if (runHistoryMenuAction != null) {
			runHistoryMenuAction.dispose();
			runHistoryMenuAction = null;
		}
	}
	...
	private void addHistoryAction(Menu menu) {
		runHistoryMenuAction = new BoundRunHistoryMenuAction(this);
		historyMenu = runHistoryMenuAction.getMenu(menu);

		MenuItem historyMenuItem = new MenuItem(menu, SWT.CASCADE);
		historyMenuItem.setText("Bind");
		historyMenuItem.setMenu(historyMenu);
	}
}