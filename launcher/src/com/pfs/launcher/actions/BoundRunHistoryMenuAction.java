...
/**
 * A specialized run history menu which binds the selected action to the associated BindableLaunchAction.
 */
class BoundRunHistoryMenuAction extends RunHistoryMenuAction {
	private BindableLaunchAction boundLaunchAction;

	public BoundRunHistoryMenuAction(BindableLaunchAction boundLaunchAction) {
		this.boundLaunchAction = boundLaunchAction;
	}
	...
	private int fillMenu(Menu menu, LaunchHistory history) {
		...
		for (int i = 0; i < favoriteList.length; i++) {
			...
			BoundLaunchActionDelegate action = new BoundLaunchActionDelegate(boundLaunchAction, launch, getMode());
	...
	private static class BoundLaunchActionDelegate extends LaunchAction {
		...
		private void bindLaunchConfiguration() {
			boundLaunchAction.setLauncherName(configuration.getName());
		}
		...
		public void runWithEvent(Event event) {
			...
			if ((event.stateMask & SWT.SHIFT) > 0) {
				if ((event.stateMask & SWT.CONTROL) > 0) {
					...
					launchMode = LaunchManager.DEBUG_MODE;
				}
			} else {
				launchMode = LaunchManager.RUN_MODE;
			}
