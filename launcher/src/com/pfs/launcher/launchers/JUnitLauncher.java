...
/**
 * Launches a JUnit test. - if the cursor is w/in a test method, runs that method - if outside a test method, runs all tests - if the class
 * declares a static method named 'suite' which returns junit.framework.Test, executes the tests returned by the suite method.
 */
public class JUnitLauncher extends AbstractLauncher {
	...
	public JUnitLauncher(LauncherConfig config) {
		super(config);
		method = getTestMethod(config.getCompilationUnit(), config.getPosition());
	}
	...
	public void run() {
		...
		if (method == null) {
			...
			StructuredSelection selection = new StructuredSelection(method);
